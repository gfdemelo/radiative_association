...
fit_transition = interp1d(transition_moment['r'], transition_moment['D'], kind='cubic', fill_value='extrapolate')
potential = np.genfromtxt('esq', dtype=[('sigma', float),('Xpi', float)], skip_header=1, usecols=(1,3))
fit_potential = interp1d(transition_moment['r'], potential['Xpi'], kind='cubic', fill_value='extrapolate')
fit_potential2 = interp1d(transition_moment['r'], potential['sigma'], kind='cubic', fill_value='extrapolate')
...
#	integrand_r = lambda b, r: abs(2.03e-6 * (((2*S + 1)*(2 - k_delta))/((2*S + 1)*(2 - k_delta))) * fit_transition(r)**2 * abs(((fit_potential2(r)) -(fit_potential(r))) * cm) **3) / ((1 - (fit_potential(r)*27.211)/E - b**2/r**2)**0.5)
	a, b = 0, np.inf
	gfun = lambda r: 3.1
	hfun = lambda r: np.inf
#        f = dblquad(lambda b, r: abs(2.03e-6 * (((2*S + 1)*(2 - k_delta))/((2*S + 1)*(2 - k_delta))) * fit_transition(r)**2 * abs(((fit_potential2(r)) -(fit_potential(r))) * cm) **3) / ((1 - (fit_potential(r)*27.211)/e - b**2/r**2)**0.5),a,b, gfun, hfun)[0] 
	#cross_section = lambda e: 4*np.pi*(reduced_mass(30.97376,31.97207)/(2*e))**0.5 * f * P  * quad(lambda b: b,0,np.inf)[0] 
	...
	rate = coeff * quad(lambda e: e * 4*np.pi*(reduced_mass(30.97376,31.97207)/(2*e))**0.5 *  dblquad(lambda b, r: abs(2.03e-6 * (((2*S + 1)*(2 - k_delta))/((2*S + 1)*(2 - k_delta))) * fit_transition(r)**2 * abs(((fit_potential2(r)) -(fit_potential(r))) * cm) **3) / ((1 - (fit_potential(r)*27.211)/e - b**2/r**2)**0.5),a,b, gfun, hfun)[0] * P  * quad(lambda b: b,0,np.inf)[0] * np.exp(-e/(pc.physical_constants['Boltzmann constant in eV/K'][0]*temperatura)), 0, np.inf)[0]
